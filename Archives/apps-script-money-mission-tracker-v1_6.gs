...
//   - Added SHEET_PROSPECTS = 'Captured Leads' tab — one row per prospect (upsert by ID)
//   - Added type='prospect_save' POST → upserts a Captured Leads row
//   - Added type='prospect_delete' POST → marks the row deleted (soft delete, audit trail preserved)
//   - Added action=prospect_log GET → returns all (non-deleted) rows as JSON
//   - Bundled v1.5 Daily Log changes so this is a single paste-and-redeploy
...
var SHEET_PROSPECTS = 'Captured Leads';   // ← NEW in 1.6
...
// 2026-05-12 — Captured Leads tab. Order matches A1XX directive headers
// (First Saved through Save Count), with ID/Last Updated/Deleted At appended
// for upsert keying and audit trail.
...
function doPost(e) {
  try {
    ...
    if (data.type === 'prospect_save') {                  // ← NEW in 1.6
    ...
    if (data.type === 'prospect_delete') {                // ← NEW in 1.6
...
function doGet(e) {
  try {
    ...
    if (e.parameter.action === 'prospect_log')        return getProspectLog(e);     // ← NEW in 1.6
...
// ── CAPTURED LEADS (NEW in 1.6) ─────────────────────────────
// Upsert one row per prospect id. Soft-delete supported via Deleted At column.
function saveProspectSnapshot(d) {
  ...
  var idCol = PROSPECT_HEADERS.indexOf('ID') + 1; // 1-based
  var last = sheet.getLastRow();
  var rowIdx = -1;
  var existing = null;
  if (last > 1) {
  ...
  var row = [
    prev.firstSaved,                      // First Saved
    d.stageName    || '',                 // Stage Name
    d.realName     || '',                 // Real Name
    d.phone        || '',                 // Phone
    d.email        || '',                 // Email
    ...
    d.hot ? 'YES' : '',                   // Hot
    d.atl ? 'YES' : '',                   // ATL
    d.firstTouch   || '',                 // First Touch
    d.nextAction   || '',                 // Next Action
    d.followUpDate || '',                 // Follow-Up Date
    d.notes        || '',                 // Notes
    prev.saveCount + 1,                   // Save Count
    id,                                   // ID
    nowIso,                               // Last Updated
    ''                                    // Deleted At (cleared on save in case of un-delete)
  ];

  if (rowIdx > 0) {
    sheet.getRange(rowIdx, 1, 1, PROSPECT_HEADERS.length).setValues([row]);
  } else {
    sheet.appendRow(row);
  }
...
function getProspectLog(e) {
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    rows.push({
      ...
      source:        r[8] || '',
      stage:         r[9] || '',
      hot:           String(r[10]).toUpperCase() === 'YES',
      atl:           String(r[11]).toUpperCase() === 'YES',
      firstTouch:    r[12] || '',
      nextAction:    r[13] || '',
      followUpDate:  r[14] || '',
      notes:         r[15] || '',
      saveCount:     parseFloat(r[16]) || 0,
      id:            r[17] || '',
      lastUpdatedAt: r[18] || '',
      deletedAt:     deletedAt
    });
  }
  logActivity('Prospect log read — ' + rows.length + ' rows (includeDeleted=' + includeDeleted + ')');
  return ContentService.createTextOutput(JSON.stringify({ status: 'ok', rows: rows }))
    .setMimeType(ContentService.MimeType.JSON);
}
...
function notionQuery(databaseId, payload, cacheKey) {
  ...
  if (cacheKey && code < 400) {
    try {
      if (text.length < 90000) cache.put(cacheKey, text, 600);
    } catch (cacheErr) {}
  }
...
function saveWeekly(d) {
  ...
  var row = [
    ...
    d.daysElapsed || 0, d.daysLeft || 0,
    d.revenue || 0, d.weekTarget || 0,
    d.adSpend || 0, d.software || 0, d.services || 0, d.otherExp || 0, d.totalExpenses || 0,
    d.netProfit || 0, d.margin || 0, d.roas || 0,
    d.dailyPace || 0, d.projectedTotal || 0,
...
function archiveCycle(d) {
  ...
  var row = [
    ...
    d.cycleNum || '', d.cycleName || '', d.cycleDates || '', d.cycleTarget || 0,
    d.daysElapsed || 0,
    d.revenue || 0, d.totalExpenses || 0, d.netProfit || 0, d.margin || 0,
    d.roas || 0, d.dailyPace || 0,
    d.sessions || 0, d.cumDms || 0, d.cumPosts || 0,
    d.studioCommission || 0, d.mixMaster || 0, d.rapreneurOS || 0,
    d.productionDeals || 0, d.customBeats || 0, d.otherIncome || 0,
    d.noteWin || '', d.noteLesson || '',
  ...
  sheet.getRange(lastRow, 1, 1, ARCHIVE_HEADERS.length)
    .setFontWeight('bold').setBackground('#0d1808');
}
...
function getBackup(e) {
  ...
  return ContentService.createTextOutput(JSON.stringify({
    status: 'ok',
    backup: { savedAt: row[0], keyCount: row[1], size: row[2], payload: row[3] }
  })).setMimeType(ContentService.MimeType.JSON);
}
...
function getHealthCheck(e) {
  ...
  try {
    var nr = UrlFetchApp.fetch('https://api.notion.com/v1/users/me', {
      method: 'get',
      headers: { Authorization: 'Bearer ' + (secret || 'none'), 'Notion-Version': '2022-06-28' },
      muteHttpExceptions: true
    });
  ...
  try {
    ...
    health.calendar = 'ok'; health.calendarCount = cals.length;
  ...
  return ContentService.createTextOutput(JSON.stringify({ status: 'ok', health: health }))
    .setMimeType(ContentService.MimeType.JSON);
}
...
function getProductionPipeline(e) {
  ...
  var items = (data.results || []).map(function(page) {
    var props = page.properties || {};
    return {
      title: readNotionTitle(props.Name), status: readNotionStatus(props.Status),
      owner: readNotionPeople(props.Assign), url: page.url, createdTime: page.created_time
    };
  });
  return ContentService.createTextOutput(JSON.stringify({ status: 'ok', items: items }))
    .setMimeType(ContentService.MimeType.JSON);
}
...
function getResources(e) {
  ...
  var items = (data.results || []).map(function(page) {
    ...
    return {
      ...
      resourceUrl: readNotionUrl(props.URL), notionUrl: page.url,
...
function getSkoolCommunity(e) {
  ...
  var result = notionQuery(databaseId, {
    ...
    sorts: [{ property: 'Opt-In Date', direction: 'descending' }],
    page_size: 100
  }, null);
  ...
  var newThisMonth = allMembers.filter(function(page) {
    var optIn = readNotionDate(page.properties['Opt-In Date']) || readNotionDate(page.properties['Date Added']);
    return optIn && optIn >= monthStartStr;
  });
  var callsBooked = allMembers.filter(function(page) { return readNotionCheckbox(page.properties['Skool Call Booked']); });
  var masterclassAttended = allMembers.filter(function(page) { return readNotionCheckbox(page.properties['Skool Masterclass']); });
  var upgraded = allMembers.filter(function(page) { return readNotionSelect(page.properties['Pipeline Stage']) === '🏆 Closed Won'; });
  var recentMembers = newThisMonth.slice(0, 10).map(function(page) {
    ...
    return {
      ...
      hot: readNotionCheckbox(props['Hot Lead']), url: page.url
    };
  });
  ...
  return ContentService.createTextOutput(JSON.stringify({
    status: 'ok',
    metrics: {
      totalMembers: allMembers.length, newThisMonth: newThisMonth.length,
      callsBooked: callsBooked.length, masterclassAttended: masterclassAttended.length,
...
function searchResources(e) {
  ...
  if (!query || !secret) {
    return ContentService.createTextOutput(JSON.stringify({ status: 'ok', url: null }))
      .setMimeType(ContentService.MimeType.JSON);
  }
  ...
  return ContentService.createTextOutput(JSON.stringify({ status: 'ok', url: url }))
    .setMimeType(ContentService.MimeType.JSON);
}
...
function readNotionUrl(prop) { if (!prop || !prop.url) return ''; return prop.url; }
function readNotionDate(prop) { if (!prop || !prop.date) return ''; return prop.date.start || ''; }
function readNotionCheckbox(prop) { if (!prop) return false; return prop.checkbox === true; }
function readNotionNumber(prop) { if (!prop || typeof prop.number !== 'number') return 0; return prop.number; }
function readNotionPhone(prop) { if (!prop) return ''; return prop.phone_number || ''; }
function readNotionEmail(prop) { if (!prop) return ''; return prop.email || ''; }
...
function getOrCreateSheet(ss, name, headers) {
  ...
  if (!sheet) {
    ...
    sheet.getRange(1, 1, 1, headers.length)
      .setFontWeight('bold').setBackground('#1a2910').setFontColor('#7ec845');
    sheet.setFrozenRows(1);
  }
  return sheet;
}
...
function authorizeNotionFetch() {
  UrlFetchApp.fetch('https://api.notion.com/v1/users/me', {
    method: 'get', headers: { Authorization: 'Bearer test' }, muteHttpExceptions: true
  });
}
...
function getInstagramData(e) {
  ...
  var posts = ((mediaData.data) || []).map(function(post) {
    return {
      id: post.id, caption: (post.caption || '').slice(0, 120),
      type: post.media_type, timestamp: post.timestamp,
      likes: 0, comments: 0, url: post.permalink
    };
  });
...
// ── SMOKE TESTS (run from Apps Script editor) ──────────────
...
function testProspectSave() {
  ...
  var resNew = saveProspectSnapshot({
    id: id, stageName: 'Trizzy', realName: 'Anonymous',
    phone: '4044045555', instagram: 'trizzymade',
    source: 'cold call', stage: 'Conversation Started',
    hot: true, atl: true, firstTouch: 'cold call',
    notes: 'needs help with mixing his EP, ATL local, hot lead',
    nextAction: 'send mix samples', followUpDate: '2026-05-15'
  });
  Logger.log('After new: ' + JSON.stringify(resNew));
  var resUpdate = saveProspectSnapshot({
    id: id, stageName: 'Trizzy', source: 'cold call', stage: 'Qualified',
    notes: 'updated: confirmed budget, ready to book session'
  });
  Logger.log('After update: ' + JSON.stringify(resUpdate));
  var resDel = deleteProspectRow(id);
  Logger.log('After delete: ' + resDel);
}
