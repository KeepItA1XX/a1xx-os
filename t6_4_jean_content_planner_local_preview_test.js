...
expect(/missionT620CreateInitialDeliveryV25\(item\)/.test(prepare && prepare[0]) && /unread:true,assignment:item/.test(html), 'initial Jean handoff enters Inbox unread before confirmation');
...
expect(/attachment_ids:\['att_'\+latest\.handoff_id\.slice\(4\)\]/.test(t616) && /attachment_id:message\.attachment_ids\[0\]/.test(t616), 'selected message owns exactly one handoff-specific typed attachment');
...
['overflow-wrap:anywhere', 'min-width:0', '@media(max-width:760px),(min-resolution:1.45dppx)', '@media(prefers-reduced-motion:reduce)', 'grid-template-columns:1fr'].forEach((token) => expect(t616Css.includes(token), `T6.16 responsive CSS includes ${token}`));
...
expect(/Ready for your routing decision/.test(t620) && /unread:true/.test(t620), 'initial delivery is unread and waiting for the routing decision');
...
expect(/existing=missionT616ContractItemV25\(item\.id\)/.test(html) && /MISSION_PHASE1_INBOX_V25\.unshift/.test(html), 'Confirm updates the canonical row while retaining generic insertion code');
expect(/existing\.unread=true/.test(html) && /existing\.state='waiting'/.test(html), 'Confirm updates the existing row to the next waiting state');
